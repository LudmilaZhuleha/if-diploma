...
const Card = ({ card, buttonType }) => {
  ...
  const renderButton = (buttonType) => {
    switch (buttonType) {
      case CARD_BTN.ORDER:
        return (
          <Button clickHandler={() => addToCart(card)} style="small">
            Order
          </Button>
        );
      case CARD_BTN.BACK:
        ...
        return (
          // eslint-disable-next-line react/prop-types
          <Button clickHandler={() => returnBookFromOrders(card.id)} style="small">
            Return
          </Button>
        );
      case CARD_BTN.CHECK_STATUS:
        return (
          <Button clickHandler={() => checkBookStatus()} style="small">
            Check status
          </Button>
        );
      default:
    ...
    <div className={styles.container_horizontal}>
      ...
      <div className={styles.wrap_horizontal}>
        ...
        {card.status === 'available' ? (
          // eslint-disable-next-line react/prop-types
          <Button style="outline_available">{card.status}</Button>
        ) : (
          // eslint-disable-next-line react/prop-types
          <Button style="outline_taken">{card.status}</Button>
        )}
        {/* eslint-disable-next-line react/prop-types */}
