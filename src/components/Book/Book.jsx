            ...
            <MainContent style="content_book">
              ...
              <div className={styles.book_info} >
                <div className={styles.starwrap}>
                  {
                    [...Array(5)].map((star, i)=>{
                      if(i < singleBook.rating) return (<Icon id="star" type="star-book-active" key={i} />);
                ...
                <p className={styles.book_release}><span>{singleBook.length}</span>, released in <span>{singleBook.released}</span></p>
                <Button style="primary" clickHandler={()=> addToCart(singleBook)}>Order</Button>
                <h3 className={styles.book_desc}>About book</h3>

                <div className={styles.book_text}>
                  {showMore ? <div dangerouslySetInnerHTML={{__html: sanitize(singleBook.description)}} /> : <div className={styles.book_text_shorten} dangerouslySetInnerHTML={{__html: sanitize(singleBook.description)}} />}
                </div>
                {!showMore ? <Button style="small" onClick={handleShowMore}>Show more</Button> : null}
              </div>
            </MainContent>
      )}
    </>
